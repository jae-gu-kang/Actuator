...
| `test_deck.js` | SASE 2026 발표 장표(`sase2026_rigging.html`) 조작 검증 |
...
루트의 `sase2026_rigging.html` (SASE 2026 추계학술대회 구두발표 장표)은 별도 테스트를 씁니다.
병합 번들에는 들어가지 않으며, `build_merged.py` 가 홈 탭에서 '논문 발표' 섹션을 잘라냅니다
(장표가 번들과 함께 이동하지 않으므로 링크가 깨집니다). 섹션을 못 찾으면 빌드가 즉시 중단됩니다.

```bash
node build/test_deck.js            # 종료코드 0=통과, 1=장표 결함, 2=테스트 자체가 깨짐
```

### 검증 항목
- **이동**: 화살표 1회 = 1장, 양 끝(첫 장·마지막 장) 경계, 쪽번호
- **역방향 정렬**: 뒤로 넘긴 뒤에도 활성 장표의 `transform` 이 `none` 인지
  (`body.rev .slide` 가 `.slide.active` 를 이겨 34px 밀리던 회귀)
- **인쇄**: 역방향 상태에서도 18장 전부 밀리지 않는지
  — **전제(`body.rev`)를 먼저 확인**한다. 앞 단계가 바뀌어 rev 가 꺼지면
  이 검사는 싸울 상대가 없어 영원히 헛돌기 때문이다.
- **목차 번호 입력**: `13` 버퍼 확정, `5` 즉시 확정, `19` 범위 초과 거부,
  **입력 도중 다른 키(리모컨 포함)를 눌렀을 때 0.9초 타이머가 살아남아
  엉뚱한 장표로 튀지 않는지** — 8가지 키에 대해
- **배치**: 18장 전부 요소가 장표 밖으로 넘치지 않는지, 콘솔 오류 0건
- **오프라인**: 외부 요청을 전부 차단한 상태에서 오류 없이 같은 배치가 나오는지

> 등장 모션 대기는 고정 시간이 아니라 `getAnimations()` 가 끝날 때까지 기다립니다
> (무한 반복 모션은 제외). 장표가 늘어나도 조용히 한계를 넘지 않습니다.
>
> 두 회귀 항목은 모두 발표 중 실제로 일어나는 조작에서 나왔습니다.
> 앞으로만 넘겨보는 테스트로는 둘 다 잡히지 않습니다.
>
> 오프라인 검사는 '외부 의존성이 없다' 를 고정할 뿐입니다. Noto Serif KR 이
> 설치된 기계에서는 대체 글꼴로 인한 배치 차이가 드러나지 않습니다.
...
검증 항목: 기본 경로 동일성(비대칭 2-패스 실제 floor 포함) · 모든 조합에서 `S.a/b/c/d` 복원
